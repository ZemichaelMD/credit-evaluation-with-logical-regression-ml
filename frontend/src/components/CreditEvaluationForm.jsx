...
export default function CreditEvaluationForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setFieldErrors({})
    try {
      const response = await axios.post('http://localhost:8000/evaluate-credit', formData)
      if (!response.data.success) {
        ...
        setEvaluationResult(response.data.data[0])
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="space-y-6 p-4">
        <Card>
          <CardHeader>
            <CardTitle>Credit Evaluation Form</CardTitle>
          </CardHeader>
